...
use crate::hal;
use crate::stm32::LPTIMER1 as LPTIM;
use crate::pwr::PWR;
use crate::rcc::Rcc;
...
impl<M: CountMode> LpTimer<M> {
    fn init(lptim: LPTIM, pwr: &mut PWR, rcc: &mut Rcc, clk: ClockSrc) -> Self {
        ...
        let input_freq = match clk {
            ClockSrc::Apb1 => rcc.clocks.apb_clk, // always enabled
            ClockSrc::Lsi => {
                // Turn on LSI
    ...
    fn configure(&mut self, conf: TimeConf) {
        // Disable the timer. The prescaler can only be changed while it's clear_bit.
    ...
    pub fn enable_interrupts(&mut self, interrupts: Interrupts) {
        // IER can only be modified when the timer is clear_bit
        self.lptim.cr.reset();
...
mod tests {
    ...
    impl TimeConf {
        ...
        fn output_period(&self, input_freq: Hertz) -> MicroSecond {
            MicroSecond(
