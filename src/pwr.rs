...
pub struct PWR(stm32::PWR);

impl PWR {
    /// Create an instance of the PWR API
    pub fn new(pwr: stm32::PWR, rcc: &mut Rcc) -> Self {
        ...
        rcc.rb.apb1enr1.modify(|_, w| w.pwren().set_bit());
    ...
    pub fn switch_vcore_range(&mut self, range: VcoreRange) {
        ...
        while self.0.sr2.read().vosf().bit_is_set() {}
    ...
    pub fn get_vcore_range(&mut self) -> VcoreRange {
        let vos = self.0.cr1.read().vos().bits();
    ...
    pub fn exit_low_power_run_mode(&mut self) {
        ...
        // // section 6.4.1.
        // self.0.cr1.modify(|_, w| w.lpr().clear_bit());
        // self.clear_lpsdsr();
    }
